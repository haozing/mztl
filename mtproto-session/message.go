...
func (m *EncryptedMessage2) decode(authKey []byte, b []byte) error {
	...
	if int(messageLen) > dbuf.Size-32 {
		fmt.Println("Message len: %d (need less than %d)", messageLen, dbuf.Size-32)
	}

	m.Object = dbuf.Object()
	if m.Object == nil {
		fmt.Println("salt: %d, sessionId: %d, messageId: %d, seqNo: %d, messageLen: %d", m.Salt, m.SessionId, m.MessageId, m.SeqNo, messageLen)
		return fmt.Errorf("decode object is nil")
	}
...
func (m *EncryptedMessage2) decrypt(msgKey, authKey, data []byte) ([]byte, error) {
	...
	if err != nil {
		fmt.Println("descrypted data error: ", err)
		return nil, err
	}
